...
module Atom = struct
  ...
  type 'a t = Atom of string * 'a Term.t list (* Atom of string * string Term.t list *)

  (** Same as [Term.map] but for all terms inside an atom. *)
  let map = fun f (Atom (a,l)) -> Atom (a, List.map (Term.map f) l)

  (** Same as [Term.convert] but for atoms.
      The returned value can be used as parameter for [Query.Atom]. *)
  let convert = fun f (Atom (a,l)) -> (a, List.map (Term.convert f) l)
